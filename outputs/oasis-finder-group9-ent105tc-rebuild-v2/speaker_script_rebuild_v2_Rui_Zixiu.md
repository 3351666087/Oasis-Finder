...
We reframed the project around the real commercial question. Oasis Finder is not just a software feature. It is a commercial method: when a merchant shows a product's supply-chain structure, does the user become more likely to buy?
...
Then we built a PySide6 control tower with dashboard, mesh, traceability, forecasting, and scenario pages. Finally, we improved the product layer: every supply-chain node can now expose stage variables and preset values.
